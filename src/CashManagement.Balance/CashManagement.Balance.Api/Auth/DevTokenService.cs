...
/// issuer, audience, exp) com o scope <c>balances:read</c>, para testar localmente
...
public sealed class DevTokenService
{
    ...
    /// <summary>Emite um token com o scope <c>balances:read</c> (caminho feliz do <c>GET /balances/{date}</c>).</summary>
    public string IssueReadToken(string subject = "dev-merchant") =>
        IssueToken(subject, JwtOptions.ReadScope);

    /// <summary>
    /// Emite um JWT válido (assinatura/issuer/audience/exp) com os <paramref name="scopes"/>
    /// informados. Permite emitir um token autenticado <b>sem</b> o scope de leitura —
